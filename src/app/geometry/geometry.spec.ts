...
describe('pxToNorm', () => {
	...
	it('converts the center point to normalized coords', () => {
...
describe('isQuadValid', () => {
	...
	it('returns false when a point is outside the image', () => {
		const outsideRect: Pt[] = [{ x: -10, y: 100 }, TR, BR, BL];
...
describe('validateQuadTLTRBRBL', () => {
	...
	it('returns ok:false for a bow-tie quad', () => {
		const bowTie: Pt[] = [TL, BR, TR, BL];
	...
	it('returns ok:false for a tiny quad', () => {
		const tiny: Pt[] = [
			{ x: 100, y: 100 },
			{ x: 140, y: 100 },
			{ x: 140, y: 140 },
